var INTERACTIVE_PLUGIN = function(exports) {
		...
		function getUID() {
			function t(t) {
				var e, n = 1,
					i = 0;
				if (t) for (n = 0, e = t.length - 1; e >= 0; e--) n = 0 != (i = 266338304 & (n = (n << 6 & 268435455) + (i = t.charCodeAt(e)) + (i << 14))) ? n ^ i >> 21 : n;
				return n
			}
		...
		function Promise(t) {
			...
			this._state = 0, this._handled = !1, this._value = undefined, this._deferreds = [], doResolve(t, this)
		}
		function handle(t, e) {
		...
		var s = 1e3,
			...
			ms = function(t, e) {
				e = e || {};
				var n = typeof t;
				if ("string" === n && t.length > 0) return parse(t);
				if ("number" === n && !1 === isNaN(t)) return e.long ? fmtLong(t) : fmtShort(t);
		...
		var ms$1 = {
			...
			debug = createCommonjsModule(function(t, e) {
				...
				function i(t) {
						...
						if (i.enabled) {
							...
							a[0] = a[0].replace(/%([a-zA-Z%])/g, function(n, i) {
								...
								if ("function" == typeof o) {
									var r = a[p];
									n = o.call(t, r), a.splice(p, 1), p--
								}
								return n
							}), e.formatArgs.call(t, a), (i.log || e.log || console.log.bind(console)).apply(t, a)
						}
					}
					...
					for (var n = ("string" == typeof t ? t : "").split(/[\s,]+/), i = n.length, o = 0; o < i; o++) n[o] && ("-" === (t = n[o].replace(/\*/g, ".*?"))[0] ? e.skips.push(new RegExp("^" + t.substr(1) + "$")) : e.names.push(new RegExp("^" + t + "$")))
					...
					return !1
				}, e.humanize = require$$0, e.names = [], e.skips = [], e.formatters = {}
			}),
		...
		function jsonp(t, e, n) {
			...
			function d() {
				i.parentNode && i.parentNode.removeChild(i), window[a] = noop$1, o && clearTimeout(o)
			}
			return p && (o = setTimeout(function() {
				d(), n && n(new Error("Timeout"))
			}, p)), window[a] = function(t) {
				debug$2("jsonp got", t), d(), n && n(null, t)
			}, t = (t += (~t.indexOf("?") ? "&" : "?") + s + "=" + l(a)).replace("?&", "?"), debug$2('jsonp req "%s"', t), (i = document.createElement("script")).src = t, c.parentNode.insertBefore(i, c), function() {
				window[a] && d()
			}
		}
		function stringify(t) {
			var e = [];
			for (var n in t) Object.prototype.hasOwnProperty.call(t, n) && e.push(n + "=" + encodeURIComponent(t[n]));
			return e.join("&")
		}
		function isArray(t) {
		...
		function isEmpty(t) {
			if (isArray(t)) return 0 === t.length;
		...
		PluginRequest.prototype.next = function(t, e) {
			...
			if ($R[e].options.isIframe && (o.isifr = 1), n && (o.djsource = n), $R[e].options.game)"scoop" == $R[e].options.game ? o.game = "6,8" : o.game = $R[e].idStore[$R[e].options.game];
			else try {
			...
			var p = "";
			return p = "https" === o.scheme ? "https://xdssp.mediav.com/s?" + stringify(o) : "http://xdssp.mediav.com/s?" + stringify(o), new Promise(function(t, n) {
		...
		var PluginReq = new PluginRequest,
			T = function(t, e, n) {
				return n = n || /\{([^\{\}]*)\}/g, t.replace(n, function(t, n) {
					var i = n.split("."),
						o = e;
					try {
						for (var r in i) i.hasOwnProperty(r) && (o = o[i[r]])
					} catch (a) {
						o = t
					}
		...
		function addEventListener(t, e, n) {
			var i = this;
			if (window.addEventListener) return t.addEventListener(e, function(t) {
				n.call(i, t)
			}, !1);
			t.attachEvent("on" + e, function() {
				var t = window.event;
				t.target = t.srcElement, n.call(i, t)
			})
		}
		function map(t, e) {
			if (isArray(t)) for (var n = 0; n < t.length; n++) e.call(null, t[n]);
			else {
				if (!isObject(t)) return;
				for (var i in t) Object.prototype.hasOwnProperty.call(t, i) && e.call(null, t[i])
			}
		}
		function sendLog(t) {
			if ("NaN" == t || !t) return !1;
		...
		Lottery.prototype.next = function(t) {
			var e = {
				...
				n = "";
			return n = "https" === e.scheme ? "https://is132ss.prod.mediav.com/s?" + stringify(e) : "http://is132ss.prod.mediav.com/s?" + stringify(e), new Promise(function(e, i) {
				jsonp_1(n, {
					param: "jsonp",
					timeout: 5e3,
					prefix: t
		...
		var lottery = new Lottery,
			...
			iframeMessageListener = function(t) {
				window.addEventListener("message", function(e) {
					if (e.data.containerID != t) return !1;
					if ("askADData" == e.data.action) {
						var n = getQueryString("fire") || "";
						$R[t].options.skin && ($R[t].pluginData.ads[0].dj_interaction = $R[t].options.skin), $R.sendMessage({
							action: "ADDataReady",
							data: $R[t].adData || [],
					...
					if ("sendLog" == e.data.action && sendLog(e.data.url), "openDialog" == e.data.action && (e.data.data.item || "none" !== e.data.data.type || investigate(1), new Dialog({
						...
						item: e.data.list,
						callback: function() {}
					}), "syncRemains" == e.data.action && (store[t].remainTimes = e.data.times, 0 == store[t].remainTimes)) try {
						var i = "",
						...
						if (a) {
							var s = a.split(",");
							i = a, s.forEach(function(t) {
								t != r && (i = a + "," + r)
							})
						} else i = r;
						window.localStorage.setItem("interactPluginGame", i), window.localStorage.setItem("interactPluginOverTime", o)
					} catch (e) {}
		...
		function getElementsByClassName(t) {
			for (var e = [], n = document.getElementsByTagName("*"), i = 0; i < n.length; i++) for (var o = n[i].className.split(" "), r = 0; r < o.length; r++) o[r] == t && e.push(n[i]);
			return e
		}
		function MediaV() {}
		MediaV.prototype.next = function(t, e) {
			...
			var i = {
				...
				o = "";
			return o = "https" === i.scheme ? "https://show-3.mediav.com/s?" + stringify(i) : "http://show.3.mediav.com/s?" + stringify(i), new Promise(function(t, n) {
				jsonp_1(o, {
					param: "jsonp",
					timeout: 5e3,
					prefix: e
		...
		function getRawTag(t) {
			...
			return e ? t[symToStringTag] = n : delete t[symToStringTag], i
		}
		...
		function debounce(t, e, n) {
			...
			function f(e) {
				var n = i,
					r = o;
				return i = o = undefined, l = e, a = t.apply(r, n)
			}
			function g(t) {
				var n = t - p;
			...
			function u(t) {
				return s = undefined, m && i ? f(t) : (i = o = undefined, a)
			}
			function b() {
				var t = now$2(),
					n = g(t);
				if (i = arguments, o = this, p = t, n) {
					if (s === undefined) return function(t) {
						return l = t, s = setTimeout(h, e), c ? f(t) : a
					}(p);
					if (d) return s = setTimeout(h, e), f(p)
				}
				return s === undefined && (s = setTimeout(h, e)), a
			}
			return e = toNumber$2(e) || 0, isObject$3(n) && (c = !! n.leading, r = (d = "maxWait" in n) ? nativeMax(toNumber$2(n.maxWait) || 0, e) : r, m = "trailing" in n ? !! n.trailing : m), b.cancel = function() {
				s !== undefined && clearTimeout(s), l = 0, i = p = o = s = undefined
			}, b.flush = function() {
		...
		function track(t) {
			...
			if (a = Math.min(a, innerHeight()), i || o || r) {
				...
				if (!isNil(s) && !isEmpty(s)) map(s.split(","), sendLog), t.removeAttribute("data-pv")
			}
		}
		function viewTrack(t, e) {
			...
			track(n, i), addEventListener(window, "scroll", debounce_1(track.bind(this, n, i), 10))
		}
		var inlay = {
			initDOM: function(t) {
				this.update(t)
			...
			initHoverDom: function(t, e) {
				...
				mediav.next(8, t).then(function(i) {
					$R[t].adData = i, "function" == typeof e ? (n.trackIconLowIE(t), e(i)) : n.trackIcon(t)
				})["catch"](function(t) {
					"function" == typeof e && e({
						error: !0
					})
			...
			sendIframe: function(t) {
				try {
					var e = $R[t].pluginData,
						n = e.ads[0].imptk;
					$R[t].clickPlugin = e.ads[0].clktk.join(","), e.impurl && n.push(e.impurl + e.ads[0].imparg), $R[t].iframe.setAttribute("data-pv", n)
				} catch (i) {}
				viewTrack(t + "-iframe")
			},
			trackIcon: function(t) {
				var e, n = $R.scheme + "://xdssp.mediav.com/s?type=14&of=4&newf=1&showid=" + $R[t].options.showid + "&uid=" + store.UID + "&r=20";
				try {
			...
			trackIconLowIE: function(t) {
				...
				if (e) {
					var n = $R[t].pluginData,
						i = n.ads[0].imptk;
					n.impurl && i.push(n.impurl + n.ads[0].imparg), e.setAttribute("data-pv", i.join(",")), viewTrack("." + t + "-thumbnail")
				}
			}
		},
			domready = function(t) {
				...
				return i.addEventListener && i.addEventListener("DOMContentLoaded", e = function() {
					i.removeEventListener("DOMContentLoaded", e, !1), s()
				}, !1), r && i.attachEvent("onreadystatechange", e = function() {
		...
		function fireworks(t) {
			...
			function s(t) {
				...
				t.width = document.documentElement.clientWidth * n, t.height = document.documentElement.clientHeight * n, t.style.width = document.documentElement.clientWidth + "px", t.style.height = document.documentElement.clientHeight + "px"
			}
			function p(t) {
			...
			function(t, e) {
				...
				for (var i = a(e, "particleCount", Math.floor), r = a(e, "angle", Number), l = a(e, "spread", Number), c = a(e, "startVelocity", Number), d = a(e, "decay", Number), m = a(e, "colors"), f = a(e, "ticks", Number), g = a(e, "zIndex", Number), h = function(t) {
						...
						return e.x = a(e, "x", Number), e.y = a(e, "y", Number), e
					}(e), u = i, b = [], x = o ? o.canvas : function(t) {
						var e = document.createElement("canvas");
						return s(e), e.style.position = "fixed", e.style.top = "0px", e.style.left = "0px", e.style.pointerEvents = "none", e.style.zIndex = t, e
					}(g), w = h.x, y = h.y; u--;) b.push(p({
					x: w,
					y: y,
					angle: r,
					spread: l,
				...
				o ? o.addFettis(b) : (t.appendChild(x), (o = function(t, e, i) {
					...
					a.save(), a.scale(t.width / p, t.height / l);
					var c = new Promise(function(e) {
						n(function c() {
							p || l || (s(t), p = parseInt(t.style.width, 10), l = parseInt(t.style.height, 10), a.restore(), a.scale(t.width / p, t.height / l)), a.clearRect(0, 0, p, l), (r = r.filter(function(t) {
								return function(t, e) {
									...
									var l, c = s / a;
									l = e.game ? $R[t].urls[e.game] : n.scene.url, addStyle(T(hoverThumbnail, {
										prefix: t,
										width: e.w,
										height: e.h,
									...
									if (e.hoverDefine.target && (d = document.querySelectorAll(e.hoverDefine.target)), !$R[t].gameBg) {
										$R[t].gameBg = document.createElement("info-div"), $R[t].gameBg.setAttribute("id", t + "-gameBg"), $R[t].iframe = document.createElement("iframe"), $R[t].iframe.setAttribute("id", t + "-popupIframe"), $R[t].gameBg.style.width = s + "px", $R[t].gameBg.style.height = c + "px", $R[t].iframe.setAttribute("frameborder", "no"), $R[t].iframe.setAttribute("scrolling", "no"), $R[t].iframe.height = $R.gameBackground.iframeHeight, $R[t].iframe.width = $R.gameBackground.iframeWidth, $R[t].iframe.setAttribute("allowtransparency", !0), $R[t].iframe.setAttribute("src", l + "?&si=" + e.showid + "&containerID=" + t + "&t=" + (new Date).getTime()), $R[t].iframe.setAttribute("data-type", e.type), $R[t].iframe.setAttribute("data-id", t), $R[t].gameBg.style.visibility = "hidden", $R[t].gameBg.style.position = "fixed", $R[t].gameBg.style.opacity = "0";
										try {
											var m = $R[t].pluginData,
												f = m.ads[0].imptk;
											$R[t].clickPlugin = m.ads[0].clktk.join(","), m.impurl && f.push(m.impurl + m.ads[0].imparg), $R[t].iframe.setAttribute("data-pv", f)
										} catch (v) {}
										document.body.appendChild($R[t].gameBg), document.getElementById(t + "-gameBg").appendChild($R[t].iframe)
									}
									function g(n) {
										// 截取
										...
										function h() {
											var t = {};
											return "left" === f.key && (t.left = f.value - s), "left" === g.key && (t.left = g.value - s), "right" === f.key && (t.left = i.right), "right" === g.key && (t.left = i.right), "bottom" === f.key && (t.top = i.top), "bottom" === g.key && (t.top = i.top), "top" === f.key && (t.top = i.top - (c - e.h)), "top" === g.key && (t.top = i.top - (c - e.h)), ("left" === f.key && "right" === g.key || "right" === f.key && "left" === g.key) && (t.top = i.top), ("top" === f.key && "bottom" === g.key || "bottom" === f.key && "top" === g.key) && (t.left = i.right), t
										}
										if ($R[t].closeIcon) {
											var u = h();
											("left" === f.key && f.value >= s || "right" === f.key && f.value >= s) && ($R[t].gameBg.style.left = u.left + "px"), ("left" === g.key && g.value >= s || "right" === g.key && g.value >= s) && ($R[t].gameBg.style.left = u.left + "px"), ("top" === f.key && f.value >= c || "bottom" === f.key && f.value >= c) && ($R[t].gameBg.style.top = u.top + "px"), ("top" === g.key && g.value >= c || "bottom" === g.key && g.value >= c) && ($R[t].gameBg.style.top = u.top + "px"), ("left" === f.key && f.value < s || "right" === f.key && f.value < s) && ($R[t].gameBg.style.left = i.left + "px", $R[t].gameBg.style.top = i.top + i.height + "px", u.top = i.top + i.height, u.left = i.left), ("left" === g.key && g.value < s || "right" === g.key && g.value < s) && ($R[t].gameBg.style.left = i.left + "px", $R[t].gameBg.style.top = i.top + i.height + "px", u.top = i.top + i.height, u.left = i.left), "" == $R[t].gameBg.style.top && ($R[t].gameBg.style.top = u.top + "px"), $R[t].gameBg.style.visibility = "visible", $R[t].gameBg.style.opacity = "1"
										} else {
											$R[t].gameBg.style.visibility = "visible", $R[t].gameBg.style.opacity = "1", e.hoverDefine.position ? $R[t].gameBg.style.position = e.hoverDefine.position : $R[t].gameBg.style.position = "fixed", e.hoverDefine.zIndex && ($R[t].gameBg.style.zIndex = e.hoverDefine.zIndex), $R[t].closeIcon = document.createElement("close-icon");
											var b = h();
											("left" === f.key && f.value >= s || "right" === f.key && f.value >= s) && ($R[t].gameBg.style.left = b.left + "px"), ("left" === g.key && g.value >= s || "right" === g.key && g.value >= s) && ($R[t].gameBg.style.left = b.left + "px"), ("top" === f.key && f.value >= c || "bottom" === f.key && f.value >= c) && ($R[t].gameBg.style.top = b.top + "px"), ("top" === g.key && g.value >= c || "bottom" === g.key && g.value >= c) && ($R[t].gameBg.style.top = b.top + "px"), ("left" === f.key && f.value < s || "right" === f.key && f.value < s) && ($R[t].gameBg.style.left = i.left + "px", $R[t].gameBg.style.top = i.top + i.height + "px", b.top = i.top + i.height, b.left = i.left), ("left" === g.key && g.value < s || "right" === g.key && g.value < s) && ($R[t].gameBg.style.left = i.left + "px", $R[t].gameBg.style.top = i.top + i.height + "px", b.top = i.top + i.height, b.left = i.left), "" == $R[t].gameBg.style.top && ($R[t].gameBg.style.top = b.top + "px", $R[t].gameBg.style.left = b.left + "px"), $R[t].closeIcon = document.createElement("close-icon"), $R[t].closeIcon.setAttribute("class", t + "-close"), document.getElementById(t + "-gameBg").appendChild($R[t].closeIcon), viewTrack(t + "-popupIframe"), addEventListener($R[t].closeIcon, "click", function(e) {
												e.stopPropagation(), $R[t].gameBg.style.visibility = "hidden", $R[t].gameBg.style.opacity = "0", $R[t].isGameBgShow = !1
											})
										}
										var x = parseInt($R[t].gameBg.style.top),
											w = document.documentElement.clientHeight;
										(x < 0 || w < c) && ($R[t].gameBg.style.top = 0), e.hoverDefine.top && ($R[t].gameBg.style.top = e.hoverDefine.top), e.hoverDefine.left && ($R[t].gameBg.style.left = e.hoverDefine.left), e.hoverDefine.marginLeft && ($R[t].gameBg.style.marginLeft = e.hoverDefine.marginLeft), e.hoverDefine.marginTop && ($R[t].gameBg.style.marginTop = e.hoverDefine.marginTop);
									...
									addEventListener(document, "click", function(n) {
										var i = n.target;
										if (i.getAttribute("class") === t + "-thumbnail") return !1;
										if ("info-div" === i.tagName.toLowerCase()) return !1;
										if ("iframe" === i.tagName.toLowerCase()) return !1;
										if (e.hoverDefine.target) {
											var o = e.hoverDefine.target.split(".")[1];
											if (i.getAttribute("class") && i.getAttribute("class").indexOf(o) >= 0) return !1
										}
										$R[t].iframe && ($R[t].gameBg.style.visibility = "hidden", $R[t].gameBg.style.opacity = "0", $R[t].isIframeShow = !1)
									})
								}
								var gameClose = function(t) {
										$R[t].closeBtn = document.createElement("close-btn"), $R[t].closeBtn.setAttribute("class", t + "-gameClose"), document.getElementById(t + "-autoWrap").appendChild($R[t].closeBtn), addEventListener($R[t].closeBtn, "click", function(e) {
											e.stopPropagation();
											var n = new Date,
												i = n.getTime(),
												o = 864e5 - (1e3 * n.getHours() * 60 * 60 + 60 * n.getMinutes() * 1e3 + 1e3 * n.getSeconds()),
												r = {};
											r.timeStamp = i, r.minMsec = o;
											try {
								...
								function initDom(t) {
									var e = [],
										...
										o = {};
									try {
										o = JSON.parse($R[t].pluginData.ads[0].dj_interaction) || {}, $R[t].gameId = o.scene.id
									} catch (d) {}
									var r, a = $R[t].pluginData.ads[0].dj_imgw,
									...
									"scoop" !== n.game && "cardMini" !== n.game && "hang" != n.game || (a = n.w, s = n.h), $R[t].iframe = null, r = n.game ? "scoop" == n.game ? o.scene.url : i[n.game] : o.scene.url;
									...
									if ("hover" === n.type || "click" === n.type) {
										...
										n.hoverDefine.target || p(c), domready$1(function() {
											inlay.initHoverDom(t), hoverIframe(t)
										})
									} else {
										e.push('<info-div style="width:{width}px;height:{height}px;" class="{id} autowrap" id="{id}-autoWrap"><iframe data-id="{id}" id="{id}-iframe" width="{gameW}" height="{gameH}" src="{url}?&si={showID}&containerID={containerID}&t={timestamp}" frameborder="no" scrolling="no" allowtransparency=”true”></iframe></info-div>'), p(T(e.join(""), {
											id: t,
											width: n.w || 500,
											height: n.h || 500,
											gameW: a,
											...
											containerID: t,
											timestamp: (new Date).getTime()
										})), $R[t].options.showClose && gameClose(t), domready$1(function() {
								...
								function lowIE(containerID) {
									initCustTag(document), $R[containerID].options.game = "hang", PluginReq.next(1, containerID).then(function(res) {
										if (1 != res.dj_outifr && 1 != res.errno && res.ads && 0 != res.ads.length) {
											...
											renderHTML(container), domready$1(function() {
												inlay.initHoverDom(containerID, function(t) {
													...
													addEventListener(document.getElementById(containerID), "click", function() {
														...
														if (t) {
															...
															$R[containerID].adData.impurl && i.item.imptk.push($R[containerID].adData.impurl + i.item.imparg), new Dialog({
																type: "bingo",
																prefix: containerID,
																item: i,
																browserName: "IELow",
								...
								function main(t) {
									if (window != window.parent && (t.isIframe = !0), t.showid) {
										...
										store.displayShowId.join(",").indexOf(t.showid) > -1 || store.displayShowId.push(t.showid), t.hoverDefine = t.hoverDefine ? t.hoverDefine : {};
										var e = namesapce + (new Date).getTime();
										console.log(e, 'test_e')
										if (store[e] = {}, $R[e] = {}, $R[e].showid = t.showid, $R[e].options = t, !timeJudge(e)) {
											if (isArray(t.game)) t.game;
											$R[e].urls = {}, 
											$R[e].idStore = {
												wheel: "1",
												wheel_browser: "1",
												...
												cardMini: "8",
												hang: "9"
											}, store.ids.push(e), is_IE(5) || is_IE(6) || is_IE(7) || is_IE(8) || is_IE(9) ? !(is_IE(7) || is_IE(8) || is_IE(9)) || "hover" !== t.type && "click" !== t.type || lowIE(e) : (PluginReq.next(1, e).then(function(t) {
								...
								return exports.render = main, exports.store = store, exports.version = version, exports
							}({});
